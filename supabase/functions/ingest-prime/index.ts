...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const secret = Deno.env.get('PRIME_WEBHOOK_SECRET');
  if (!secret) {
    console.error('[ingest-prime] PRIME_WEBHOOK_SECRET not configured');
    return new Response('Internal Server Error', { status: 500 });
  }

  if (!verifySecret(req.headers.get('x-webhook-secret'), secret)) {
    console.warn('[ingest-prime] Invalid webhook secret');
  ...
  try {
    const result = await ingestFormSubmit(body, 'prime');

    // Fire-and-forget: trigger n8n enrichment pipeline
    const n8nUrl = Deno.env.get('N8N_WEBHOOK_URL');
    if (n8nUrl && result.leadId !== 'unknown') {
      fetch(n8nUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...body, leadId: result.leadId, brand: 'prime' }),
      }).catch((err: unknown) => console.error('[ingest-prime] n8n error:', err));
    }
    ...
    console.error('[ingest-prime] Unhandled error:', msg);
    return new Response(
